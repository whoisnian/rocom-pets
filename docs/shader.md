...
**这条流水线也适用于 NPC 那套材质图(`M_C_*`)**,2026-08-11 靠它一次读出两条
(详见 rocom-pets 的 docs/design.md「NPC 也能出包」那几节):

- **`M_C_Eyes`(目标 PS 51613)**:瞳孔 UV 是 `frac(uv × (2, 1))`,再按 `uv.x ≤ 0.5`
  在左右两组「缩放/平移」之间选。**那个 2 是字面量**,而实例里同时摆着一个
  `UVOffsetScale = 2` —— 它其实只乘平移那一项。**名字像不像和它是不是,是两回事**;
  这个材质刚好两边都是 2,读反了照样能修好这一只。
- **`M_C_Object`(目标 Low PS 49710)**:自发光/辉光两层的遮罩是 `MatID` 贴图的
  **B 通道**做 `saturate((b − 0.04) × 1.1111)` —— 与宠物 `M_P_Object`(PS 68952)
  同一个重映射,但那边取的是**基色 alpha**。同一张图还给出 `材质ID = 1 + 5×(1−R)`。

后者顺带补上一条**贴图槽的旁证**:「t 槽 ↔ 贴图参数名」两条路仍然断着(见上面),
但**贴图内容本身能反过来认**:t3 紧接着做 `1 + 5×(1−R)` 五档 ⇒ 它必须是 `MatID`
(实测那张图的 R 只有 0/76/127/178/255 五个台阶);t2 的 R、G 紧接着 `×2−1` ⇒ 切空间法线
⇒ 它是 `MaskTex`(实测 `_M` 的 R/G 中位数都是 0.498)。**下游怎么用,就说明上游是什么。**
...
- **身上那层星点(`MI_P_Object_XingGuang_FakeTrans01`,shader 27803,7 个 ub ⇒ 材质 cb 是 cb6)**
  ...
  **这一层的运行时门:两族两条,都读出来了(2026-08-04)**。起因是 rocom-pets 收到实机反馈
  「春兔身上几乎看不到星点」,而按当时的判据它该有。把三条 shader 拉平了比:

  | 材质 | shader | 门 |
  |---|---|---|
  | `M_P_Object` | 51377 / 7959 / 8628 | `if (法线图.a >= 0.4)`,**整段星点在这个 `if` 里** |
  | `MI_P_Object_XingGuang_FakeTrans01` | 27803 / 34270 | 同上(同族同一段代码) |
  | `M_P_Object_Trans` | 51670 / 8752 | `RampID >= 0.4`(`cb6[84].z`),**无分支,当 0/1 系数乘** |

  「`t3` 是法线图」不是猜的:采完紧接着 `mad r3.xy, r3.xy, 2, -1` → `nz = sqrt(1-x²-y²)` →
  与切线基 `v0/v1` 组合,标准切空间法线;`.w` 是美术塞在 alpha 里的遮罩通道。
  半透族那条的名字按上面②的判据配到块 9(V=83、`83+⌈142/4⌉+1 = 120 ≥ 声明的 cb6[119]`),
  `cb[84].z = 标量 6 = RampID`(根默认 0);**交叉验证**:同块 `cb[88].z = StarStickTiling = 4`,
  正是两条指令后星点 UV 乘的那个槽。

  半透族里这个门罩着**两处**:一是 `max r1.z, r1.z, r8.w` 那条 alpha 链
  (`r1.z` 的来历是 `saturate((基色a - 0.04) * 1.1111)` —— `Opacity or OpacityMask` 那个重映射),
  二是高光层;~~一度以为它「只改不透明度、不画彩色星」~~ —— **那是漏看了分支**:
  第 319 行 `if_nz r2.y`(`r2.y` 就是第 172 行那个门)里边第 490~500 行还有一份
  完整的四段渐变 `lerp(r0, Stick_Intensity × m × c, saturate(m + GlassyMainColorOpacity))`。
  **教训还是那条**:切片切到「这个寄存器进了 alpha」就收手,会漏掉同一个门罩着的另一支;
  一个门管几处,得把它的**每一个消费点**都找出来。

  ⇒ 又一次印证上面那条:读完一条公式,先问它属于哪一族、在那一族里接到哪个累加器。
...
- **「解不出冻结块」有两种,别混为一谈**(2026-08-04,追果冻外壳的不透明度时分清的):
  ① `Special/` 那种是**名字补丁表的形状不同**(上一条),破法是去兄弟材质上读同一段代码;
  ② 果冻那种是**内容压根不内联** —— 分辨方法很硬:**看有没有长度对得上的 preshader 链**。
  果冻的 `_By` 与 `MI_P_Object_Trans_WPO` 都确实拥有 shader 68869(map 里有),
  而它要 `V = 89`(汇编末向量槽 88、`dcl cb6[127]`),两个 uexp 里**最长的非零起链只有 39 条** ——
  长度对得上的链根本不存在,名字补丁表也一条没有(把 `patch_tables` 的 `cnt`/`num`/`ns`
  四种组合都放宽过,仍是 0)。**这种放宽锚定没有用**,和 `GLASS_RIM_GAIN` 卡的是同一堵墙。
  ⇒ 遇到 ② 只剩安卓那条路(见 rocom-pets 的 docs/android-device.md:
  求值 preshader 直接出 UB 数值,不需要参数名)。
