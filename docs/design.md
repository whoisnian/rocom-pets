...
| **球内那颗星在实机里是黄金色、又大又居中** | 我们代的是根默认 `StarColor` (0.33, 0.67, 2.0) —— **偏蓝**。原来文档记的「偏蓝而实机 near-white」也不准,**实机是黄的**。所以那个颜色不该取 `StarColor`,或者它被别的槽覆盖了 |
...
| **玻璃球的两条新差别**(对着实机那颗球看出来的) | ① 实机的球**几乎没有明暗渐变**(一片平色 + 一小块白高光 + 居中一颗星),我们有很强的斜向分界;② 球内那颗星实机是**黄金色**,我们代 `StarColor` (0.33,0.67,2.0) 出来偏蓝 —— 原来记的「偏蓝而实机 near-white」也不准。②说明那个颜色不该取 `StarColor` | 小~中:①要么是环境项相对直接光更强、要么取景差异,可比对多个 yaw 定;②要找出真正喂那颗星的槽位 |
