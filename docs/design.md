...
2. **同物种多实体必须共享资产**。**模型侧已做**:`PetActor.model` 改成 `Arc<Model>`,
   ...
   **实测:1 只 RSS 234MB、3 只 237MB**(每多一只 ≈ 1.3MB,就是它自己的画布与回读缓冲),
   共享前应是按只数翻倍。调试开关 `ROCOM_PETS_ENTITIES=3` 用来在托盘接上之前验这条路。
   **还留着的**:`replace_actor()` 仍只换第一只(切形态只影响它),
   `actor()`/`actor_pos()`/`is_dragging()` 也还是单实体过渡访问器 —— 等第 7 步做托盘的
   「加一只/移除一只/切形态」时一并清掉。
