...
当前基线(21 只,越接近 1.00 越好;色偏是 0.00):
    ...
    中位(19 只;红绒十字与雪影娃娃因实机侧抠图不可比被排除):
    有投影( 9 只)  亮度 0.96  调色板 0.065  描边 1.09  对比 0.80
    无投影(10 只)  亮度 0.94  调色板 0.084  描边 0.96  对比 0.93
    全部 (19 只)  亮度 0.96  调色板 0.070  描边 0.99  对比 0.80
...
**落地了半边**:导出器给半透族加上 `RampID >= 0.4`(`Materials.TransStarGate`)。
效果 —— 春兔 `_Fx` **关**(与实机一致)、果冻 `_By`(`RampID` = 0.5)**保留**、
小甲虫 / 牵线木偶 / 一窝蜂 / 女王蜂 / 艾诺菈的半透件全关、暮星辰 `_Fx1` 关
(逐像素差 0.118%、最大 14,四项指标一位没动)、幽星光与曜星光 **0 变化**。
**全库真正把这一层开着的,只剩果冻那一个材质。**
...
1. **不透明族那道逐像素门**没实现 —— 要在 shader 里采法线图的 alpha。
   **量过之后决定现在不做(2026-08-04)**:半透族那道落地后,全库还带这层的只剩
   ...
   既然它只值 ±0.002,不追。)
2. ~~**半透族这一层「只改不透明度、不画彩色星」**~~ —— **这条我写错了,已更正**:
   ...
   所以「果冻该不该有彩色星点」的答案是:**该有**(`RampID` = 0.5 开着),
   而实机截图里看不见它 —— 这条留在下面「星点仍然过重」那节。
...
| **果冻的外壳圆顶还是太透** —— **链读出来了,卡在「给槽位定名」** | 「少了上半身」剩下的那一半:外壳 `_By` 在圆顶那块 UV 的基色 alpha 是 **0.388**,我们照着画 0.39,于是背景透进来、里面那颗不透明的内胆成了主体(转 25° 时眼睛会落到内胆轮廓外)。读了它自己那条 shader(**68869**,材质 cb6、`dcl cb6[127]`、V=89)：最终 alpha 是 `saturate(基色链 × r1.x + r0.w)`,**多出来的 `r0.w` 是加性的**,里头有一段方向/高度渐变:<br>`h = dp3(v1, cb6[28])×0.5+0.5` → `t = saturate((h − cb6[124].x)/(cb6[124].y − cb6[124].x))` → `lerp(cb6[124].w, cb6[124].z, t)` → `× cb6[101].z`,最后整体 `× max(cb6[81].x, cb6[81].y)`。这解释了实机那圈为什么是实心的 | ~~**卡点**:两个 uexp 都解不出冻结块,**先把 uniexpr 的块边界锚定放宽**~~ —— **查过了,不是锚定的问题,放宽也没用**:这条 shader 的 `FUniformExpressionSet` **压根不在包里**。证据链:① 68869 确实属于 `MI_Wat_BuDing1_001_By` 与 `MI_P_Object_Trans_WPO`(两边的 map 里都有);② 它要 **V = 89**(汇编:末向量槽 88、`dcl cb6[127]`);③ 而这两个 uexp 里**最长的非零起 preshader 链只有 39 条**,连长度对得上的链都不存在 —— 名字补丁表也一条都没有(`patch_tables` 放宽 `cnt`/`num`/`ns` 四种组合后仍是 0)。这和 `GLASS_RIM_GAIN` 卡住的是同一堵墙:**内容不内联**。<br>⇒ 剩下的只有**安卓那条**(见 android-device.md:实机私有目录里的 resource + 求值 preshader **直接出 UB 数值、不需要参数名**)。**在那之前别去凑系数** —— 很暗占比 0.124 对实机 0.052 就是这一条 |
...
| **果冻内胆的 glassy flow 只用了两色均值** | 汇编是 `lerp(lerp(FlowColor02, FlowColor01, 噪声), FresnelColor, 菲涅尔)`(见 §1.1),我们没有那层噪声流动,拿两个 flow 色的均值当平的替身 | 小:全库只有 1 个材质走 `M_ShuiMu_ByIn`,加一张噪声图 + 两次 lerp 即可 |
