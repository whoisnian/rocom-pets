...
| **把 `glow` 那几层也搬进线性** —— 剩下的 HDR 值都挡在这儿 | `glow`(通用边缘光、玻璃的 MatCap 与边缘光、星点)仍在显示空间,因为它的系数都是在那儿标的:`sqrt` 编码会把小值放大(0.25 → 0.418),硬搬过去全库过曝 9 → 23。搬过去之后 `MatCapColor` 的 HDR (2,1.76,1.45)/(3,3,3) 与边缘光色才能原样代进来 | 中:要连 `GLASS_MATCAP_GAIN` / `GLASS_RIM_GAIN` / `STICK_GAIN` 一起重标,每项都得对着截图确认 |
