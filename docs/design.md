...
4. **`glass = RedChannel × MainTex.r + GreenChannel × MainTex.g`**,再 `× StarIntensity`。
   ——玩家选的那两个颜色就是这两个参数。
...
`cb6[49]`(星点色)、`cb6[62].x`(星点覆盖偏置)、`cb6[62].y`(整层混合,材质里叫
`BlendWeight` = 1.0)。这个材质实例的冻结块凑不出 65 槽的那份(它只有 35 槽与 48 槽两种),
所以这三个是**按语义接的**,不是查出来的;`src/pet/glassy.rs` 的常量注释里逐个标了。
其余全部对得上名字,而且有**三条独立证据同时指着同一组**:汇编里的结构、lua 覆盖的参数
名单、以及 `HIDDEN_GLASS_CONF` 每条参数自带的中文 `num_param_comment`
(反射强度 / 纹理缩放深度 / X轴流动速度 / 法线 / 贴图缩放 / 颜色细节)。
