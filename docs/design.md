...
- **存成表情包(2026-09-05 补)**:选好这一身(形态 / 眼神 / 异色 / 炫彩 / 朝向缩放 /
  正在播的那段动作)之后点「下载 GIF」,把**眼前这一身**的一整个动作循环存下来。
  - **不是把画布录下来**:画布只能不透明(见上一条)、rAF 的采样点和动作时长没关系
    (录出来首尾接不上)、而且画布是长方的。所以另开一张**方的 RGBA 离屏纹理**,
    时间轴按 `t = i × 时长 / 帧数` 均匀取,清成全透明 ⇒ 拿到的是带真 alpha 的帧
    (`Preview::capture`)。
  - **抓帧那一路也超采样 2 倍**(`STICKER_SS`),理由同桌宠画布 —— 管线一个采样点都没有。
    缩回目标尺寸是**在 GPU 上**做的(一个全屏三角形 + 线性采样),所以回读量与贴纸尺寸
    一致而不是它的四倍。
  - **着色器那个 `time` 冻住**:火焰流动、星点闪烁的周期和动作时长没有公倍数,跟着推进的话
    每绕一圈就跳一下。表情包是无限循环的,宁可让那几层停在一个相位上。
  - **界面上默认只有一个「下载当前动作为表情」按钮**,按下去才展开尺寸 / 帧率 / 透明背景 /
    下载,同时画布上出现**取景框**。框是必需的:三个设置里「尺寸」影响的正是框里那块,
    没有框就调了也看不出变化。框 = 「画布正中、边长 = 画布短边」的那个方块
    (CSS 里 `width: min(100cqw, 100cqh)`,父元素开 `container-type: size`),
    与 `Preview::capture` 里那个 `fit = min(w,h)/h` 是同一条 —— 横屏按高、
    竖屏(手机上弹窗会变窄)按宽,不然会把画布外的东西抓进去。
  - **帧率是挑的**(20/25/50),因为「更流畅但文件更大」是个人偏好。**只放能整除 100 的档**:
    GIF 的延迟以百分之一秒为单位,`1000/fps` 除不尽的话一个循环会偏几个百分点。
    帧数与延迟都由 `sticker.ts` 的 `plan` **从厘秒倒着算**,于是循环总时长就是这段动作的时长。
  - **一次抓不下就分批**:512² 一帧 1MB、上限 400 帧 ⇒ 420MB,标签页扛不住。
    `capture(size, total, first, …)` 一次给一批(返回这批几帧),`encodeGif` 逐批要;
    而且**抓两遍** —— 第一遍只凑调色板的取样,第二遍边抓边写 GIF,于是内存只压一批。
    重抓很便宜(60 帧 512² 回读 103ms),结果也逐位相同(`seek` 到定点、`time` 冻住、相机不动)。
  - 上限 400 帧定的是**最低帧率**:全库最长的运行时动作是 12.27 秒(放松),
    原来 60 帧封顶时它只有 4.9fps —— 实机反馈的「掉帧严重」就是它。
  - **GIF 只有 1 位透明**,所以「透明」那档得自己把 alpha 二值化(门限 128)并**去预乘**
    —— 不去预乘的话边缘会压着一圈黑。想要羽化的边就选纯色背景那档:
    `rgb + 底色 × (1 − a)` 正是预乘合成,边缘能正常过渡。编码器是 `gifenc`,
    **按需 import**(9KB 一个 chunk,只想下包的人一个字节不付),全片共用一张调色板
    (逐帧各量化一次会让静止的地方在相邻帧上跳色号,看着整只在闪)。
