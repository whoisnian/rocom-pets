...
**亮度那一档确实补不回来,但原因不是雾,是我们没有 HDR 余量。** 抬曝光能把亮度比拉到
0.98(曝光 0.70)、调色板 0.084、对比 1.03 —— 四项全线更好,**但全库过曝从 3 暴涨到 98**。
游戏在 HDR 里算完再削顶,削掉的很少;我们的贴图是 LDR,抬曝光只是削顶。所以这一档
要等「贴图也进 HDR」才有解,不是调参数能动的。
...
当前基线(17 只,越接近 1.00 越好;色偏是 0.00):
    ...
    中位: 亮度 0.83   调色板 0.088   描边 1.02   对比 1.12
