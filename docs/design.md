...
| 项 | 值 | 定于 | 状况 |
|---|---|---|---|
| 暗部系数 `mix(0.72, 1.0, lit)` | 0.72 | 最初 spike + `466326f` | **灰度结构是对的**(下面单列一条),只有 0.72 这个数没验过 |
...
`mix(暗, 亮, lit)` 这个**灰度结构本身是对的**,A 类里要复核的只是 0.72 这个数值。

**同时排除了一条路**:对这一项「按值锚定」走不通 —— 父/根材质的 uniform expression set 里
这对的值是**中性占位**(实测都是 1.0,渲出来等于不打光),真值是每只宠物的 MI 覆盖,
所以绕不开参数名。它跟「名字这一关」一起解冻。
...
| 还卡着的具体条目 | **只剩颜色了**:两段明暗那四对明/暗色 (24,25)/(28,29)/(32,33)/(36,37)、球内星点的颜色(现在代的是根默认 `StarColor`,出来偏蓝而实机 near-white)、球体固有色色相偏一档。**结构与量纲的部分都已按汇编落地**:内部层已开、march 深度按 `GlobalDepth` 算、闪烁按 G 通道相位 | 都等上面那条 |
