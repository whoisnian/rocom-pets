...
| **折射必须在物体空间算,不是世界空间。** 汇编里视线先被 `cb2[6..8]`(`Primitive` 的 world→local 矩阵)转进物体空间,法线取的也是物体空间的那份;我原来整段在世界空间算 —— 于是星**钉在视线上**,球自转时星不跟着转,看着「像一层屏幕投影贴上去的星形」(用户实测) | 运行时用**蒙皮矩阵的逆**做等价变换:刚体骨骼(旋转+平移、无缩放)下 `inverse(mat3(m)) == transpose(mat3(m))`,而那两颗球正是各自挂一根骨头刚体自转,所以准。顶点里多带两个插值量:未蒙皮法线 + 物体空间视线 |
