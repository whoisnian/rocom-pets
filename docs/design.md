...
当前基线(17 只,越接近 1.00 越好;色偏是 0.00):
    ...
    中位(15 只;红绒十字与雪影娃娃因实机侧抠图不可比被排除):
    亮度 0.93   调色板 0.077   描边 0.95   对比 1.01
...
- **配置窗口第一版满屏豆腐块**。egui 自带字体只有拉丁字母,而我为了躲开「ab_glyph
  读不了字体集合」的顾虑把 `.ttc` **整个跳过了** —— Linux 的 Noto CJK、Windows 的雅黑与
  宋体**全都是 `.ttc`**,一份都没找到。epaint 0.35 实际走 `skrifa::FontRef::from_index`,
  字体集合是支持的,`FontData` 就有 `index` 字段。**而且下标不能省**:
  `NotoSansCJK-Regular.ttc` 里装着日/韩/简/繁四套字面,取错只会静悄悄显示成日文字形。
- **`fc-match "sans-serif:lang=zh-cn"` 的第一名一个汉字都没有**(`NotoSans-Regular.ttf`,
  「sans 的最佳匹配」)。只按 `:lang=zh-cn` 问才对,顺带拿到正确的字面下标。
...
- **性格 → 表情的规则是从解包数据里查出来的**,不是编的:
  ...
  换成游戏里的八条,名字与 `nature_id` 照抄,便于回表核对。
