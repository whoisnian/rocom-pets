...
| 项 | 值 | 定于 | 状况 |
|---|---|---|---|
| 暗部系数 `mix(0.72, 1.0, lit)` | 0.72 | 最初 spike + `466326f` | **灰度结构是对的、汇编那对值也读出来了(0.5 / 1.5)**,但代不进来 —— 缺曝光那一环,见下面那张表 |
...
**那对值也读出来了:亮 = 1.5、暗 = 0.5**(`MI_P_Object_Trans_MatCap` 的 shader 20284,
`cb6[60]`/`cb6[61]`,标量 #29 / #32;被 `r6 * r11 + r13` 消费,r11 是那对高度渐变色)。
**但代不进来** —— 三条实测把原因钉死了:

| 试法 | 圆顶中位(实机 (255,197,242)) | 误差和 | 冲白占比 | 全库过曝 |
|---|---|---|---|---|
| 现状 `mix(0.72, 1.0)` | (238,143,200) | 111 | 0.001 | 9 |
| 汇编原值 `mix(0.5, 1.5)` | (255,209,255) | **25** | **0.304** | **109** |
| 只取暗部 `mix(0.5, 1.0)` | — | — | 0.001 | 7 |
| 只取比值 `mix(0.333, 1.0)` | (238,142,199) | 115 | 0.001 | — |

- 原值把中位数误差从 111 压到 25,但圆顶中央 30% 冲白(实机 0%),全库过曝从 9 涨到 109。
- 只取暗部虽然全库更干净,但**对比比实机更强**:实机圆顶几乎看不出方向性明暗,我们反而更花。
- 只取比值比现状**还差** —— 差距根本不在暗部,在**亮面**:我们封顶 1.0,到不了实机的亮度。

**所以真正缺的一环是曝光/显示编码。** 实机先在 HDR 里乘 1.5,再由 shader 尾部的
`movc o0.xyz, (曝光 < 1), sqrt(色 × 曝光), 色` 编码到显示空间,削顶后就成了实机那种
「大片饱和的淡色 + 少量暗部」;我们整条链跑在显示空间、没有这一步。
**这一项要等曝光那一环补上再动**,已记入横向待办。
...
| **补上曝光 / 显示编码这一环** —— 现在挡着好几项的**新总闸** | 实机是「在 HDR 里算完、再由 shader 尾部 `movc o0.xyz, (曝光 < 1), sqrt(色 × 曝光), 色` 编码到显示空间」;我们整条链跑在显示空间、没有这一步,于是**凡是大于 1 的材质值都代不进来**。已经被它挡住的:两段明暗那对(汇编读出的是亮 1.5 / 暗 0.5,代原值全库过曝从 9 涨到 109,见 §1.1 那张表)、`MatCapColor` 的 HDR 值 (2,1.76,1.45)/(3,3,3)、边缘光色。**换句话说 A/B 两类里好几项不是「不知道值」,是「知道了也用不了」** | 中:要把贴图当 sRGB 解码进线性、整条链改线性、末尾统一编码,还要重标那几个手挑系数;全库回归会整体位移,得逐项确认 |
| 还卡着的具体条目 | **两段明暗那对值已经读出来了(1.5 / 0.5),但等曝光那一环**(见上一行)。剩下真正还缺名字的:球内星点的颜色(现在代的是根默认 `StarColor`,出来偏蓝而实机 near-white)、球体固有色色相偏一档、星点层那 4 个渐变色槽 | 名字那几项等下面那条;明暗那项等曝光那条 |
