...
| 角色 | 形态 | 动作 | 声音 | 运行时实渲 |
| --- | --- | --- | --- | --- |
| 芙蕾雅 | `NPC_07401` / `NPC_08501` | 12 / 11 | 无库 | 干净 |
| 可丽希亚 | `NPC_10801` | 15(含坐下当睡觉) | 7 段 | 干净 |
| 乐乐 | `NPC_03801` | 12 | 无库 | 干净 |
| 可丽希亚 | `NPC_01301` | 15 | 7 段 | **右腿被甩出体外** |

**那条腿:上游 ACL 解码的锅,只报不改。** `NPC_01301` 的 17 根有蒙皮的骨骼在**每一段动画里**
都被「恒定地」挪出绑定姿势,数值 86 段一模一样,最大 `Bip001-R-Foot` 115cm;
而那些「常量」正好等于**别的骨骼**的参考姿势平移(`Bip001-L-Foot` 拿到的是 `Bip001-R-Thigh` 的)。
排除法做完了:网格骨骼 109 根、骨架骨骼 109 根、**参考姿势逐根一致(差 0 根)**,
`World_Idle` 的轨道映射是**恒等**的 107 条 —— 我们这侧的索引没有错位,
问题在 `CUE4Parse-Conversion` 那条 ACL 路径(`nReadACLData` + `offset = trackIndex * numSamples`)。
同角色的第二套 `NPC_10801`、芙蕾雅、乐乐都没有这毛病(只有裙摆/救生圈那几根 14~24cm 的小幅命中)。

**没有自动纠正,是故意的。** 能想到的每一种规则(「恒定平移就退回绑定姿势」)都会误伤真的
恒定平移 —— 可丽希亚坐下那一段(`SleepLoop`)的根骨骼就是**恒定压低 68cm**,那是对的。
改成在 `GlbBuilder` 里**检测并报到 `report.txt`**:门槛 10cm、只算真的有顶点权重挂着的骨骼
(定位骨 `locator_ball_1`、乐乐的 `Bone_Yu_01` 权重为 0,写歪了也看不见),每个形态汇总一条、
带最大偏移量。调完之后**只有 `NPC_01301` 报警,另外四个形态静默** —— 信噪比够用了。

**顺带修好的**:`--probe-anim` 原来用**网格**的骨骼序去查轨道,而轨道是按**骨架**的骨骼序排的
(CUE4Parse 三条填充路径都是 `for boneIndex in 0..skeleton.BoneCount` + `FindTrackForBoneIndex`)。
两者顺序可以不一样,于是探针报出来的 Δ 是「这根骨骼」对上「另一根骨骼的轨道」。
现在改用骨架序,并额外打两行:网格与骨架的参考姿势差几根、轨道映射是不是恒等 ——
这次就是靠这两行把嫌疑从「我们索引错了」洗掉的。

**宠物侧零回归**:同一个二进制重导 002-喵喵,与已发布的 `.rkpet` 逐字节比 ——
**glb 与 png 全部相同**,只有 ogg 的 Ogg 流序列号(ffmpeg 每次随机)与 manifest 里
一行注释文案不同。
