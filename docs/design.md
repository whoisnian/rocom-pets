...
| **`_D` 贴图的 alpha 不是不透明度**，是美术塞的遮罩通道。813 张 `_By_D` 里 160 张 alpha 通过率 <95%、60 张 <5%(火花 4.8%、迪莫 0.39%)，而喵喵恰好全 255 | 渲染**不能**拿 `tex.a` 做 alpha 测试，轮廓只由几何决定。喵喵是当初唯一肉眼验过的宠物，所以这个 bug 藏了很久 |
| **`_Fx*` 材质槽有两种用途**，只能按几何占比区分：装饰(几个三角的加色光晕)vs **本体**(火花 79%、幽星光 92%、小鼓象 91% 的身体就是 Fx 层做的)。122 个形态带 Fx | 运行时按占比取舍：<40% 当装饰丢掉，≥40% 当本体保留 |
...
| 伸展/张翅/跳跃的姿势明显超出**绑定姿势**包围盒：120 个抽样形态里 11 个按绑定盒取景会裁掉肢体 | 取景用「每段动作采样一遍」的并集包围盒(`Model::motion_bounds`)，尺寸换算仍用绑定盒(站姿高度不能随动作变) |
...
2. **贴图 alpha 被当成不透明度**:`if tex.a < 0.35 { discard; }` 把 160 张 `_By_D` 里的身体
   啃掉一部分,火花只剩眼睛、迪莫整只消失(见 §1)。alpha 是遮罩通道,不是 opacity。
3. **取景按绑定姿势包围盒**,伸展类动作被裁(120 个抽样里 11 个)。改成按动作包围盒取景后
   0 个被裁,而且取景余量从 1.35 降到 1.15,画布反而更小了。
...
| **半透材质** | 水蓝蓝那类半透水体:Fx 是内外两层壳(三角数一模一样)+ 一张噪声贴图，靠半透混合出水感。当本体保留就画成一团噪声，丢掉又只剩蝴蝶结和脸，**两头都不对**。全量抽查里目前只见这一类 | 中：要加半透管线与排序，且依赖上面那条解出混合模式 |
