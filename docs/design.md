...
| **`StarStick` 是「贴在镜头上」而不是「贴在表面上」**:实机里星点不随模型转动,像镜头前挂了一层遮罩投到宠物身上;而且**一只宠物只有一份**(各材质自己写的平铺数不一样,照各自画就成了两种密度叠在一起) | 采样坐标取 **NDC**(取景视体是正方的,格子不会被拉扁,平铺数就是「横跨模型几格」);导出时把一个形态里的平铺数统一成一份,优先用明写了 `StarStickTiling` 的那个 |
