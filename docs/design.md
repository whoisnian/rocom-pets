...
| **解 `FUniformExpressionSet` 的 FMemoryImage 布局** —— 剩下材质问题的**总闸**,已走到半程 | **结构和默认值已经能读**(rocom-capture 的 `scripts/uniexpr.py`):记录格式、`Index==-1 && Association==2` 的识别特征、preshader 头链、以及 cb 的分区(向量在前每条一个 float4、标量紧接其后 4 个装一个)。**只差名字**:冻结镜像里名字字段是空的、靠块内相对的补丁表打进去,而实例的包名字表只有它自己覆盖过的参数名(要从根材质取)。踩过的四个坑记在 [rocom-capture/docs/shader.md](https://github.com/whoisnian/rocom-capture/blob/master/docs/shader.md) | 中:剩下就是认出根材质里那张「(delta, 名字下标)」表的编码。打通后链路已明确:`UniformVectorPreshaders[k]` → opcode → `Parameter(idx)` → `UniformVectorParameters[idx]` |
| 还卡着的具体条目 | 两段明暗那四对明/暗色 (24,25)/(28,29)/(32,33)/(36,37)、球的固有色色相偏一档、星点强度的确切系数、内部层的深度与平铺(内部层因此还关着) | 都等上面那条 |
