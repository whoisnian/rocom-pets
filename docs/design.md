...
| **星点层的强度是 `Stick_Intensity`(根默认 1.5),平铺就是材质的 `StarStickTiling`** —— 名字都查实了(参数名哈希)。运行时那个手挑的 `STAR_TILE_SCALE = 3.0` **已经删掉**(现在是 1.0),平铺纯粹来自材质 | 原来 3.0 的理由是「星点比实机大一倍以上」,**方向正好反了**:放大对照实机截图才看清 —— 实机是**少而大**的四角星(整只身上三四颗),我们是**多而小**(十几二十颗)。定下 1.0 + 整体系数 0.6(× 1.5 = 净 0.9)是照实机放大图挑的 |
