...
**这一节是给宠物侧看的摘要,已单独并进 master(`c834377`)。** 三条都是在 NPC 分支上查出来的
(那边中招的形态多、症状大),但**判据里没有任何 NPC 特有的东西**,宠物库照样命中;
逐条是怎么定下来的见后面「可丽希亚的右腿」与「菲尔特待机少一只脚」两节。
全库 201 个包 / 617 个形态重导逐字节比:**614 个逐字节不变**,变的 3 个逐张渲图看过、都是改好。
...
### 菲尔特待机少一只脚:那份错位映射其实**写在资产里**,不必推(2026-08-11)

实机反馈「菲尔特的待机少了左脚,点行走/奔跑又正常」。查下来是**两个**毛病叠在一起,
而且第一个把上面那套推断法整个比下去了。
...
`TrackToSkeletonMapTable` 是「第 i 条轨道属于哪根骨骼」的权威答案。压缩器会把整段恒定的
骨骼(道具、末端 Nub)整条剔掉,于是轨道数少于骨骼数、映射非恒等 —— **这种 CUE4Parse
处理得是对的**。把菲尔特(`NPC_01401`,87 根骨骼)90 段动画按「轨道数 × 映射」摊开:

| 轨道数 | 映射 | 段数 | |
| --- | --- | --- | --- |
| 87 | 恒等 | 16 | 一根不少,对 |
| 84 | 非恒等 | 6 | 剔掉 `Bone_Bottle01`/`Bone_Cork01`/`Bone_Spoon01` 三根道具骨,对 |
| 84 | **恒等** | **67** | **坏的** —— `World_Idle` 在里面 |
| 79 | 非恒等 | 1 | |

第三行自相矛盾:恒等 = 「剔掉的是末尾那三根」,而末尾三根正是
`R-Calf`/`R-Foot`/`R-Toe0`(整条右小腿)。那 6 段好的给出的映射是
`轨75→骨78, 轨76→骨79(L-Thigh), … 轨83→骨86(R-Toe0)`,而坏掉的 `World_Idle`
**数据恰好也是这个排布**:逐根比首帧,轨79 的旋转与 `L-Toe0` 的参考姿势差 1°、
轨80 与 `R-Thigh` 差 12°、轨83 与 `R-Toe0` 差 1°,没有第二根骨骼能对上。
两条独立证据指同一份映射 —— **那就别推了,直接借过来用**(`BorrowTrackMap`)。

按**轨道数**配对而不是一个资产一份:同一个资产不同动作剔掉的骨骼数不一样,可丽希亚
(109 根)有 107 条的一族(剔右臂两根 twist)与 106 条的一族(剔的是另外三根),
而**同一个轨道数下的映射逐条相同**(107 条那 6 段一致,106 条那 4 段也一致)。
同一个键下两段不一致就整条作废,退回 `DetectTrackShift`。

**这也回头验证了上面那套推断**:可丽希亚 107 条那份真映射说 `轨81→骨83`,
而 `DetectTrackShift` 当初推出来的正是「第 81 根起错 2 位」—— **一模一样**。
菲尔特这边它推成了「第 77 根起」,晚了两根:76、77 那两根的真值不是参考姿势
(道具骨在动、大腿在转),判别不出来,于是扫描停在了最后一次命中的地方。
少修的那两根里就有 `Bip001-L-Thigh`。

顺带把一条老代码删了:`if (FindTrackForBoneIndex(bone) < 0) continue;`。轨道表比骨骼少几条时
它对**末尾那几根恒返回 -1**,而借来的映射说它们的数据就在最后几条轨道里 ——
菲尔特的整条右小腿就是这么被整段丢掉的(渲出来右腿僵在绑定姿势)。

**② 缩放那一路根本没错位 —— 按错位读,靴子就没了。**

修完①靴子还是不见。量下来:那 49 段错位动作**每一段都恰好给两根骨骼写缩放**,
值分毫不差地固定在 0.0978 与 0.0976;按错位读会落到 `Bone032` 与 **`Bip001-L-Calf`** 上 ——
左小腿缩成十分之一,靴子整只缩进裤腿里。**「少了左脚」不是脚没画,是脚缩掉了。**

那两个值属于 75 / 77 号骨骼(`Bone_Bottle01` 与 `Bone_Spoon01`),三条证据:

- 干杯(`GanBei`,映射本来就对的一段)里 `Bone_Bottle01` 的缩放**就是 0.0978**;
- 另外 21 段映射正确的动作里,`Bone032` 与 `L-Calf` 一次都没被缩放过 —— 真是它俩的话
  不会 49 段全有、21 段全无;
- `World_ShaoZi_Idle`(「拿着勺子」的待机,87 条轨道、映射正确)**不缩放任何骨骼**,
  而普通 `World_Idle` 缩放 77 号 —— 一个拿着勺子、一个把勺子藏起来,正好对上。

为什么单单缩放不错位:ACL 把旋转/平移/缩放分三份存,三份**各自**把「整段恒定」的轨道剔掉。
道具骨的旋转平移恒定(被剔了)而缩放不是(要靠它藏起来),所以缩放那份没被压缩、
下标仍然对着骨骼序。所以错位的动作里 **旋转平移读错位后的轨道、缩放读本骨骼那一格**。

**还得顺手把「没有旋转平移轨道但有缩放」的骨骼留下**:道具骨自己整条被剔了,原来一律
`continue` 掉,那样酒瓶和勺子会一直挂在身上。留下之后绝大多数仍然一条通道都不写 ——
**这些道具在绑定姿势里本来就是缩到 0.098 的**(藏着),恒定且等于绑定值的通道会被裁掉;
真正写出来的只有 `GanBei` 那种「把瓶子拿出来」的段。

### 全量 60 个包之后收拾的两处(2026-08-11)

`--npc-all` 全导一遍(60 个包 / 70 个形态)之后,哨兵在 34 个形态上报了警。拆开看是两类,
各自处理:

**① 停在十米开外的道具骨 → 拉回绑定姿势。** 23 个形态属于这类:远行商人的武器
`Bone_wuqi_U_01` 在 **10.5 米**外、约十个角色的 `Bip001-Prop1` 在 1~2.7 米外。
这多半是美术把用不到的道具挪出画面藏起来 —— 游戏里靠蓝图切可见性,而蓝图我们读不到,
留着就是十米外飘着一件东西,还会把包围盒撑大(远行商人的预览里角色被缩成画面顶上一小点)。

判据是**「整段恒定 + 偏出四分之一身高」**(`GlbBuilder.ParkedThreshold`,再兜 30cm 下限),
**只对 NPC 开**。
恒定 = 这段动画根本没在移动它;而超过身高的四分之一,也不可能是「摆到手边一点」。
按身高而不是绝对值:同样偏 40cm,对 1.8 米的人是半条腿,对 40cm 的小个子是三个身子。
命中就退回绑定姿势 —— 道具回到美术挂它的那只手上,看着是有意为之。

顺带把罗兰战斗演出里那条偏 178cm 的 `Bip001-R-Calf` 也一并收了(同一个判据)。

**~~本来想全库一起开,量完改成只对 NPC~~。** 扫了已发布的 201 个包 / 617 个形态
(直接读 `.rkpet` 里的 glb,找「单关键帧平移通道且偏离绑定姿势超阈值」的),
**34 个形态、304 条通道会被碰到,而抽查下来它们是对的**:伏地兽那条伸出去 57cm 的舌头
(`Bone_shetou_001`)正是这只宠物的样子,鸭吉吉挂在身上的炸弹、阿瓦鲨嘴边的小鱼也一样。
宠物个头小(阈值常常落在 30cm 下限上),「四分之一身高」在那边太紧;NPC 全是一米八的人形,
同样的判据只会捞到十米外的武器和解码坏掉的小腿。**第一版的小样本(喵喵 + 石肤蜥)
恰好一条都没命中,差点把这条当成「零回归」放过去** —— 全库扫描才是该做的验法。

### 幽灵版艾什莉整个人只剩一对眼睛(2026-08-11)

全库 70 个形态跑 `tools/sweep.py`,唯一一个「空白」是艾什莉(`NPC_02602`)——
渲出来只有两只眼睛飘着。

材质族是 `M_Character_Base_TG`(材质名带 `_Gho`,幽灵版),实例**同时**写着
`Opacity = 0` 与 `Alpha = 0.8`,另有一整套 Fresnel 参数(`FresInt`/`FresExp`/
`FresColorUp`/`FresColorDown`)撑那层鬼火边缘。我们按 `Opacity` 取覆盖度,于是身体全透。
**一个带完整基色 + 遮罩贴图的材质不可能真是 0 覆盖**,那个 0 是这一族没用上的输入 ——
这一族改读 `Alpha`(`MaterialInfo.IsCharacterGhost`)。修完是个半透明的蓝裙子女孩,
`sweep.py` 的「空白」归零。

**~~剩下的 3 个「过曝」是固有的~~**:~~大星灵本身就是一颗白星星、白树女神一身白、
恩佐的觉醒形态是白发白衣~~ —— **三个里有两个是真 bug**,见下面「自发光那层的遮罩」。
真正固有的只有大星灵(一颗白星星)。当时没有实机对照,就把「白」当成了设定;
`sweep.py` 的数字**只说这一格白得反常,不说白得对不对** —— 拿它当「已知无害」的依据是错的。

**② `Fight_*` 不再进「其余全收」那一批。** 11 个形态的**身体骨**报警,而坏的**全在这一族**:
罗兰的 `Fight_Command` 整条腿的平移是别的骨骼的,渲出来身体折叠炸开;同一个角色的
`World_*` 一段都没坏。这一族的错位量还**不均匀**(`L-Calf` 差 2 位、`R-Thigh` 差 3 位、
`R-Calf` 干脆无法归因),所以 `DetectTrackShift` 那条「整批一致才动手」的判据够不着它。

战斗入场演出(出场/亮相/下令/战败/应对)桌宠本来就用不上,**少一段动作好过多一段炸开的**。
`CallOut` 不受影响 —— 它走别名表,源就是 `Fight_CallOut`。真要看那批就 `--all-clips`。

顺带的好处:一个形态少 9~11 段,乐乐的 glb 11.1MB → 7.6MB、可丽希亚 19.8MB → 16.1MB。
...
### 一整套 NPC 内斜视:两只眼睛各占半张贴图,平铺那一步在着色器里(2026-08-11)

实机反馈「大部分 NPC 看起来都是内斜视」。**这条不是材质参数的事,是我们少做了一步采样。**

网格摊开来看就明白了(露西亚 `NPC_01201` 的 `_Es` 那一片,152 个顶点分成四个连通块):

| 连通块 | 顶点 | 世界 x | UV u |
| --- | --- | --- | --- |
| 右眼球 | 64 | −0.110 … −0.015 | 0.015 … 0.484 |
| 左眼球 | 64 | +0.015 … +0.110 | 0.515 … 0.984 |
| 高光小面片 ×2 | 12 ×2 | 两侧 | 0.011 … 0.032 |

**两只眼球各占半张贴图,而瞳孔贴图上只画了一只、居中**(那只眼睛横跨 u ≈ 0.32~0.68)。
照 UV0 原样采,左眼只取到 [0.32, 0.484]、右眼只取到 [0.5, 0.68] —— 各自只剩瞳孔**靠鼻梁**
的那一小条,于是整套 NPC 一起内斜视。

补上的那一步在 `M_C_Eyes` 的像素着色器里(按 docs/shader.md 那条流水线取出来的目标
PS 51613):

```text
mul r1.xy, v3.xyxx, l(2.0, 1.0, 0, 0)    ← u 平铺 2、v 不动
frc r1.xy, r1.xyxx
...
**那个 2 是字面量,不是参数** —— 这一点必须查实,因为实例里明晃晃摆着一个
`UVOffsetScale = 2`,顺手拿来当平铺看着天衣无缝。它其实只乘**平移**那一项
(`cb6[28].x` 配到 `scalar-slot[8]` = `scalar-param[8]` = `UVOffsetScale`),
是「眼球看向哪儿」的增益;全库实例的 `L_/R_ Move/Scale` 八个数都是 0,所以实机剩下的
只有平铺。名字读反了照样能修好这一只,但换个 `UVOffsetScale ≠ 2` 的角色就会错。

**烘进顶点,不放运行时**(`GlbBuilder.BakeEyeUv`):整段只与顶点 UV 有关、不含任何运行时
状态,而两只眼球在 UV 上是互不相交的岛,没有三角形跨 u = 0.5 那道缝。烘完
`verify_glb.py` / `sweep.py` 看到的就是实机的样子,运行时也不必为这一族开分支。
逐眼的缩放/平移仍照公式一起烘,免得哪只不是 0。

**踩了一脚**:改完 UV 忘了 `accessor.UpdateBounds()`。那两个 min/max 是 glTF 的必填元数据,
SharpGLTF 写盘时拿它校验,对不上直接抛 `Accessor[79] memory: Value[0] is out of bounds`。
全量 70 个形态里 **11 个当场导不出来**(乐乐、万事通、罗兰、亚瑟存档…),而剩下 59 个
恰好因为烘完的范围仍落在原区间内,一声不吭地过了 —— 只看抽样的那几只完全看不出来。

顺带加了一道守卫:这一片的 UV0 若与非眼睛材质**共用同一条 accessor** 就跳过并警告。
上游给每个 section 各写一份顶点(眼睛 152、身体 5428),但共用是 glTF 完全合法的写法,
真共用了还照改就是把身体的 UV 一起翻倍。全量跑下来这条警告 0 次。

### 自发光那层的遮罩:宠物在基色 alpha,NPC 在 `MatID` 的 B 通道(2026-08-11)

实机反馈三条,查出来是**同一个 bug**:

- 恩佐(`NPC_09601`)只有脸和武器正常,其他区域纯白 —— 身体材质 `Emitter Intensity = 5`,
  实例没写 `Emitter Color`,退到根默认的**白**;
- 白树的部分服装纯白 —— `_Im` 那个槽白 × 3;
- 露西亚脸部之外的区域偏紫 —— 身体写着 `Emitter Color = (1, 0, 1)`(品红)× 10。

宠物那边这一层的遮罩是**基色 alpha 的重映射**(`saturate((a − 0.04) × 1.1111)`,
查实于 `M_P_Object` 的 PS 68952,水灵的条纹与火神的爪尖都对得上)。**而 NPC 的基色贴图
alpha 恒为 1** —— 照同一条路走,遮罩恒等于 1、整片刷满。

`M_C_Object` 是另一张材质图,遮罩换了来源(目标 Low PS 49710):
...
**「t3 就是 MatID」是验出来的,不是按槽位猜的**(t 槽 ↔ 贴图参数名那条路仍然断着,
见 docs/shader.md):紧接着那步 `1 + 5×(1−R)` 与这一族的 `ColorID2..5` / `RampID2..5` /
`SpecIntensity2..5` **五档**一一对应,而实测 `T_*_ID` 的 R 恰好只有 **0 / 76 / 127 / 178 / 255**
五个台阶(露西亚 92% 的像素落在其中四个上)。同一趟还确认了 t2 = `MaskTex`:
它的 R、G 紧接着做 `×2−1` 解码,是切空间法线 —— 与实测「`_M` 的 R/G 中位数都是 0.498」吻合。

改完那三张 `_ID` 的 B 通道有 93.7~100% 是 0,真该亮的只有恩佐袍子上那 **0.73%** 的符文。

**强度对不上本来就是信号**:宠物全库只有两个材质开着这一层(0.3 / 0.4),而 NPC 这边
12 个槽的强度是 1 / 2.5 / 3 / 3 / 5 / 8 / 10 / 10 / 10 / 11 / 11 / **30**。乘数到 30 只在
「遮罩几乎处处为 0、亮的是一小条灯带」时才讲得通 —— 当时没往这儿想。

**按根材质分族,不按资产目录**:恩佐的 `_Im2` 挂的是宠物那张 `M_P_Object`,照旧走基色 alpha。
判据是 `MaterialInfo.IsCharacterFamily`(父链上有 `M_C_*`)。宠物库 201 个包 / 1771 个材质槽
扫下来 **`M_C_` 出现 0 次**(21 种根材质全是 `M_P_*` 与几个一只一份的定制族),
这两条新路径对宠物**结构上不可能触发**。

包格式加了一个可选字段 `emissive_mask_tex`(旧包没有 ⇒ `None` ⇒ 老行为),
运行时多一条贴图绑定;`pet.wgsl` 里 `detail_mask` 的来源由 `material.emissive_mask.x` 选。
