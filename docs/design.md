...
4. **有些宠物有异色形态**(用户指出)。机制找到了:根标量 `MutationColorSwitch` +
   每宠物的 `RedChannel` / `GreenChannel` / `BlueChannel` + `MaskTex` 的 R/G/B 分区
   (火娃3 上 `OpenNRCMask` = 开),外加 `Mutation_MatCap`。**我们没实现这套**,
   所以拿实机截图当参考前要先确认那张不是异色版。已记入横向待办。
...
| **异色(变异)形态** | 用户指出菊花梨、红绒十字等都有异色版。机制在材质里:根标量 `MutationColorSwitch` + 每宠物的 `RedChannel`/`GreenChannel`/`BlueChannel` + `MaskTex` 的 R/G/B 分区(火娃3 上 `OpenNRCMask` = 开)+ `Mutation_MatCap`。**导出器与运行时都没处理**,而且它还是「拿实机截图当参考」时的一个混淆源 | 中:换色本身不难(遮罩分区 × 三个颜色),难在从配置里认出哪些宠物有异色、以及桌宠这边怎么选 |
