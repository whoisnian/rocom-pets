...
粒子图四张,隐藏款各带一对)。塞进 201 个包要多背 120MB;打进二进制会把 18MB 的产物顶到
21MB,而且**素材不该进代码仓库**。做成包目录**旁边**的一份 `glassy/`(3.6MB),
`rocom-pets-export --glassy` 导一次,**已经导好的包不用重导也能用上炫彩**。

赛季款给 4 只宠物配的专属 `MainTex` 刻意不导:那是整张身体图,7 张就把 3.6MB 顶到 34MB,
