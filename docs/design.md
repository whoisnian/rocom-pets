...
| **有基色的材质也可能是半透**:暮星辰的裙子(`Fx1`)与那两个球(`Fx2`)都是 `MI_P_Object_Trans_*` 家族、`BLEND_Translucent`;球还带 `MatCapColor=(3,3,3)` 的 HDR 白高光与紫色 `Rim LightColor` | 混合模式对**所有**材质生效,不能只看纯特效层。半透的一律进特效通道(后画、不写深度),边缘更实中间更透。当不透明画的话球就是两个纯色圆片 |
...
| **顶点色存在但是遮罩通道**:四个通道各自 0–1、均值只有 0.2,配合材质里的 `UseVertexColorG` / `RedChannel`/`GreenChannel`/`BlueChannel` 使用 | 直接当颜色乘上去会整体发黑。暮星辰环带的青→粉渐变大概出自这套,但要猜通道语义与混合公式,**暂未实现** |
