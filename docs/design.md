...
| 项 | 值 | 定于 | 状况 |
|---|---|---|---|
| 暗部系数 `mix(0.72, 1.0, lit)` | 0.72 | 最初 spike + `466326f` | 汇编那里是**颜色对** `cb5[24]/[25]`,不是灰度系数;名字未解 |
| 明暗过渡 `smoothstep(-0.04, 0.04, ndl)` | ±0.04 | 同上 | 汇编的阈值在 `cb5[58].w`/`cb5[59].x`(相邻标量 #19/#20),名字未解 |
