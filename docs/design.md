...
| **但 shader 反过来认不出材质**:archive 里没有任何材质名(搜 `M_P_Object`/`XingGuang` 零命中);同目录的 `NRC_PCD3D_ES31.stable.upipelinecache` 里也只有哈希、没有 stable 字符串键;DXBC 的反射段 `RDEF` 被剥了,只剩 `ISGN`/`OSGN`/`SHEX` | 唯一还留着名字的是每条 shader 尾部的 uniform buffer 名(`View`/`Primitive`/`Material`/`MobileBasePass`/`ClusteredForwardShading`…),只够粗分类。要落到「哪条属于 `MI_Ill_XingGuang1_001_Fx1`」得从材质资产的 cooked resource 里读 shader map 的 SHA1,再来 archive 的 `ShaderMapHashes` 里查 —— 还没做 |
...
| **matcap 图均值只有 0.2、只有 8% 的像素亮过 0.5**(matcap26 / Matcap35 实测):暗区是球体自己的暗面 | 只取「减掉 0.35 再归一化」的那部分当高光。连暗区一起加等于整片抬一层灰,再乘上 HDR 的 `MatCapColor`(暮星辰的球 = (3,3,3))就冲成一团白 |
...
| **玻璃不吃两段明暗**:那两颗球是开口薄壳(129 顶点、30 条边界边),自转时露出来的面一直在换,硬分亮/暗两段就让整颗球在 0.72↔1.0 之间跳 | 玻璃族的明暗只来自反射(MatCap + 边缘光),`lambert` 直接取 1 |
| **那两颗球里那颗单独转动的小星星,在导出的资产里根本找不到**:`Fx1` 只有 2 个连通分量(两颗球各 129 顶点、都是正球),材质挂的每一张都查过了 —— `BaseTex` 与 `MaskTex` 在球的 UV 处都是平色圆盘、`MainTex`(`Tex_PetGlassy_007_D`)是红绿双通道的斑点噪声、`MatCap`(matcap26/35)是玻璃球高光图、`StarStickTex` 是五角星色块图集;资产目录下也只有 SKM + 材质 + 动画 | 那是运行时另外挂上去的特效组件(粒子/socket 挂件)—— 也正好解释「球内的形状会单独转」:精灵始终朝着镜头,球在它后面自转。要支持得走蓝图/VFX 导出那条路,**目前画不出来**,已记入横向待办 |
...
| 反编译宠物材质的 shader | 剩下所有拧不动的都指向它:那 11 个真半透材质的不透明度从哪来、幽星光那颗琥珀球为什么实机是红的、星点强度的确切系数、边缘光到底怎么参与。DXBC 已经能整条取出来(见 §1) | 大:还差两步 —— ① 材质 → shader map SHA1 的归属;② DXBC 反汇编器(RenderDoc / 3Dmigoto cmd_Decompiler / DXVK,都要另装) |
