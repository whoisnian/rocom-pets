...
### 头像图标按 conf_id 命名,和 manifest 是同一套 id(2026-08-09)

`Icon/HeadIcon/<conf_id>.png`,128px 一张,解包树里 **634 张**(另有 `3001_2.png`
这种带后缀的备用图,和 manifest 对不上,不收)。而 manifest 里 `[species].id` 与
`[[forms]].id` 就是 conf_id —— 下载站的头像因此**按 id 直接对上**,不必拿中文名去查。

这条把下载站从「取隔壁 rocom-petvo 的成品精灵图 + 按中文名查序号」换成了
「自己从解包数据拼」。按 id 比按名字好在两处:同名不同图鉴号的宠物各有各的头像
