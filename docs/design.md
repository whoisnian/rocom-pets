...
| 8 | 部分支持宠物叫声 | ✅ 四个触发点各一段,按 `voice` 属性变调;全库 835 个形态里 499 个有 |
...
- **导出器直接从 pak 里读 WwiseAudio**(`exporter/Voice.cs`)。那 3.3GB 就在包里,provider
  手上已经有 —— 不必像 rocom-petvo 那样先侧解一份出来。bnk 解析是 `scripts/wwise.py` 的
  最小 C# 移植:FNV-1(**先转小写**)拿 Event id → Action → 容器 → Sound 的 `sourceID`,
  下行**必须靠 `directParentID` 反建父子树**(rocom-capture 那边踩过:按「扫描 4 字节命中
  已知 id」的启发式会一路爬到 ActorMixer 根,三个事件返回同样的 67 个 wem)。
- **一个形态一套四段**,对上 §7 的四个触发点:`Common_Happy`(摸头满意)、`Common_Shock`
...
- **变调 = 变速**。游戏里每只宠物有个 −100~100 的 `voice` 属性,喂给 Wwise 的
  ...
  曲线两端写进 manifest,**每只实体开台时随机一个 `voice`** —— 同物种两只听着不一样。
...
- 默认音量 0.35(常驻程序必须小声),托盘里一个「叫声」勾选临时静音,`volume = 0` 干脆
  不开音频设备。**不做 BGM**。
