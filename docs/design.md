...
| **视线相关的采样层会「区域白闪」**:玻璃内部层的采样圆盘随视线扫过星场,扫到亮星就是一块白斑闪一下 —— 宠物一转就闪 | 这类效果**要么参数全对、要么别开**。内部层已按此关掉导出(代码与查实的采样起点都留着);关掉后 30 帧「同时转 yaw + 推进动作」的扫描里,接近纯白的像素占比均值 0.00%、峰值 0.05% |
...
| **仍未解决:cb 槽位 ↔ 参数名。** shader 里 `cb5[32]`/`cb5[33]` 这种是**编译顺序**的 `VectorExpressions`,和 `CachedExpressionData` 的顺序是两套 | 要对上得解 shader map 里的 `FUniformExpressionSet`(preshader 操作码流,里头引用的也是哈希名)。所以「哪个 cb 槽是哪个参数」还得靠**从汇编读出的语义**去反推(如「唯一的折射比 = `GlobalRefraction` = 1.3」) |
...
| 球旋转时**仍有区域白闪**(用户实测,比之前轻但没消除) | 上一轮定位到一个来源(玻璃内部层的视线相关采样)并关掉了,峰值白像素从可见降到 0.05%,但**没有消除**。剩下的嫌疑按可疑度排:① **屏幕空间星点遮罩** —— 球又小又快地掠过一层固定在屏幕上的遮罩,星点在球上必然忽明忽暗,和内部层是同一类问题;② **MatCap 高光** —— 那两颗球是开口薄壳(129 顶点、30 条边界边),自转时露出来的面一直在换,而 matcap 图上是硬边的白色方块 | 小:两条都能靠「单独关掉那一层再跑 30 帧白像素扫描」定位,不用猜 |
...
| 读剩下的 shader 汇编 | 还没解释的:那 11 个真半透材质的不透明度从哪来、幽星光那颗琥珀球为什么实机是红的(应该就是两色渐变)、星点强度的确切系数、第二张三向投影贴图那层 | 中:纯分析活。cb 槽位 ↔ 参数名仍未打通(编译顺序与 `CachedExpressionData` 顺序是两套),只能靠汇编里的语义反推 |
