...
| ~~`LINE_BOOST` 乘法提亮~~ **形状已按汇编改对** | 加性 0.2 | `383182c`/`466326f` | 罗隐 body shader 51377 第 99~103 行:`r1.w = saturate((基色.a − 0.04) × 1.1111)` 然后 `mad r6.xyz, cb6[7].xyzx, r1.w, r6.xyzx` —— 是**往固有色里加一个颜色**,不是乘亮度倍数,而且用的是那个已知的 alpha 重映射。只剩强度是标定的(`cb6[7]` 的名字未解)。全库 `过曝` 1 → **0** |
...
当前基线(17 只,越接近 1.00 越好;色偏是 0.00):
    ...
    亮度 0.93   调色板 0.082   描边 0.98   对比 0.86
