...
**复验后的两条追加:幽火的「双层」是画家序,水灵条纹的颜色是白不是 `Color1`(2026-08-08)**
...
改成「进只测深度、不写深度的那一遍」之后一次对上:外壳先画且不写深度,内壳随后照样通过
深度测试直接盖上去,两层都在、而且各自不透明。运行时按 `paint_order` 走,
导出器按根材质名 `M_Gho_XiaoYou_GhostFire` 认(与 `M_Gra_Yutu_Ear_Lighting` /
...
**把这一层单独隔离出来**再比 —— 同一份包只改这一个参数渲两张、相减,得到的就是这层本身:
    ...
    线上提升(显示空间)        R     G     B
    实机(x=641 实测)         +96   +23   +9
    `Color1` × 0.5(上一版)   +21   +11   +10      ← 色相偏蓝、强度只有三分之一
    **白 × 0.5**              +43   +15.6 +9.0     ← 比例与实机一致(峰值 +94/+50/+31)

根因在导出器:`EmissiveColor` 原来只认**实例显式写的** `Emitter Color`,而水灵/波波拉的
水体层只写了 `Emitter Intensity`、颜色留在根默认(白)⇒ 整层被当成「没有自发光」丢掉;
上一版为了补救才去接 `Color1`。现在改成「实例没写就退到根默认」,并撤掉
`IsWater ? 0f` 那个清零(它拦掉的是同一层)。全库因此有 **105 / 2355** 个材质带上这一层。

**复量**:25 只对照里暗影灵面 0.268 → **0.253**、幽冥眼 0.203 → **0.200**、
曜星光 0.084 → **0.076**、波波拉 0.149 → 0.147、水灵 0.112 → 0.111,
