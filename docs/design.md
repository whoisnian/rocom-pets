...
| **星点共享图 `Tex_PetGlassyStar_004` 的形状不在 alpha 里**:它是张区域图集,红/橙/黄的随机色块、每块中间一颗浅蓝白小星,**alpha 恒为 255**(整张 rgb 均值 (218,119,31)) | 强度取**蓝减红**:底色块 R 高 B 低、星芒反过来,相减只剩星芒。按 `rgb * a` 算等于把整张橙图糊到表面——暮星辰的裙子从饱和蓝被冲成彩虹糖就是这么来的 |
| **`StarStick` 是「贴在镜头上」而不是「贴在表面上」**:实机里星点不随模型转动,像镜头前挂了一层遮罩投到宠物身上 | 采样坐标取**视空间的横/纵世界坐标**(正交投影下正比于屏幕像素,而且天然是方的)。用 NDC 会被画布长宽比拉扁,用视空间法线则会跟着转身跑 |
...
| **`Rim Power` 小于 1 时那不是「一圈边」而是整片泛色**:幽星光那两个球是 0.35,整颗都该是红的;而且材质给的是 `Rim LightColor`/`Rim DarkColor` **一对端点** | 边缘光按**混色**而不是加色:`mix(固有色, RimColor, pow(1-N·V, RimPower))`。当加色画的话 R 早饱和到 1、再加只抬 G/B,红球会泛成橙黄 |
| **那两颗球里单独转动的小星星不在导出的骨骼网格里**:`Fx1` 只有 2 个连通分量(两颗球各 129 顶点),两块 UV 都是**平的橙红渐变、alpha 全 0**,材质里也没有任何星形贴图;资产目录下只有 SKM + 材质 + 动画 | 那是运行时另外挂上去的特效组件(粒子/附加网格),要支持得走蓝图/VFX 导出那条路。**目前画不出来**,已记入横向待办 |
