...
| **球的固有色是两个颜色参数按物体空间高度做的渐变**:`lerp(cbA, cbB, smoothstep(高度))`,高度取 `物体空间 y - 包围盒 min`,再乘一个对比度重映射 | 这解释了为什么我按 UV footprint 取平色圆盘,幽星光那颗琥珀球对不上实机的红 —— 固有色压根不来自基色贴图 |
...
| 读宠物材质的 shader 汇编 | 剩下所有拧不动的都指向它:那 11 个真半透材质的不透明度从哪来、幽星光那颗琥珀球为什么实机是红的、星点强度的确切系数、边缘光到底怎么参与。**取用与归属已全部打通**(见 §1),`ps_5_0` 汇编能直接出 | 中:纯分析活。要读几百行汇编,并把 cb 槽位对到参数名(靠 `CachedReferencedTextures` 的顺序 + 已知的参数值反推),而且里头混着大量光照代码 |
