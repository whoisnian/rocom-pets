...
| --- | --- | --- | --- |
| 1 | 置顶于普通窗口之上（浏览器/终端/文件管理器） | ✅ 截图确认压在桌面内容之上；`layer=top` 按协议本就在普通 xdg-toplevel 之上 👀 再拿窗口压一次 | ⏳ |
...
| 4 | 精灵内点击/拖动由自己接到 | ⏳ 👀 逻辑与输入区已就位（`Stage` 有单测覆盖拖动/命中） | ⏳ |
| 5 | 精灵外的点击落到下层窗口（不吃掉） | ⏳ 👀 输入区 = 轮廓的 24 个矩形近似，区域外按协议不派发 | ⏳ |
| 6 | 运行时切换全局穿透后，精灵内的点击也落到下层 | ✅ 机制侧：`SIGUSR1` 后输入区在 24 ↔ 0 个矩形间切换（日志确认）；👀 落点效果 | ⏳ |
...
**Wayland 半边：架构维持。** 最关键的三件事——layer-shell 拿到置顶铺满的表面、
`PreMultiplied` 逐像素 alpha、输入区可运行时切换——都实测通过，没有需要改架构的发现。
