...
pub fn render(request: &Request) -> Result<()> {
    ...
    // 光向。原来是 `(-0.4, 0.8, 0.6)`,注释写着「和直觉一致」—— 一个猜值,而且**X 的符号是反的**。
    //
    // 实机的主光方向**读不出来**:它走 MaterialCollection 的 `C_MainLight_Direction`,
    // 而那个参数在 `MPC_S_Global` 里的默认值是 `(0,0,0,0)` —— 运行时由蓝图写入的状态,
    // 不是资产数据(见 rocom-capture `docs/android-glsl.md`)。
    //
    // 但它是**拍摄参考截图时的真实物理量**,而且 15 只共享同一个,所以可以从截图**估**出来
    // (估参不是调参:一个方向要同时拟合 15 只,可证伪)。扫下来调色板中位:
    //
    //     (-0.4, 0.8, 0.6) 0.077   ← 原值
    //     ( 0.4, 0.8, 0.6) 0.060       ( 0.4, 0.5, 0.9) 0.060
    //     ( 0.6, 0.6, 0.5) 0.068       ( 0.25, 0.9, 0.5) 0.060  ← 取它(对比比最好)
    //     ( 0.0, 1.0, 0.0) 0.076
    //
    // **调色板与对比是一对真实的权衡,别再扫了**:调色板要到 0.060 必须有足够的
    // **+Z(正面光)**;把 Z 压下来对比回得来,但调色板也退回基线附近,等于白改 ——
    //
    //     光向              亮度  调色板  描边  对比
    //     (0.25, 0.90, 0.50) 0.95  0.060  0.96  0.87   ← 取它
    //     (0.18, 0.92, 0.40) 0.95  0.076  0.97  0.88
    //     (0.25, 0.95, 0.20) 0.93  0.074  0.96  0.94
    //     (0.12, 0.95, 0.30) 0.93  0.073  0.97  0.97
    //     (-0.4, 0.80, 0.60) 0.93  0.077  0.95  1.01   ← 原值
    //
    // **翻个 X 就从 0.077 掉到 0.060**,而且在 +X 一带是平底、逐只普遍变好
    // (加益 0.101→0.046、学院呱呱 0.066→0.036、点点 0.051→0.030、魔力猫 0.049→0.032)。
    // 这不是噪声,是原来那个符号错了。
    //
    // **代价要记着**:对比比 1.01 → 0.87(我们比实机更平),全库过曝 4 → 5。
