...
pub enum Control {
    ...
    /// 配置窗口那张动作表点出来的。
    Play(u32, u32),
    /// 重新读配置与阵容存档,把台上的一切对齐过去。配置窗口改完什么都发这个。
    ///
    /// **不重扫包目录** —— 切形态、调大小、改性格都走这条,而它们跟包目录无关;
    /// 把整个包目录的 manifest 读一遍是热缓存 40ms、冷缓存 400ms,白付。
    Reload,
    /// 同上,但**连包目录一起重扫**。只有三种时候该发:`--reload`(人手动要求的,
    /// 那就该看见磁盘上的最新状态)、配置窗口里导入了包、删除了包。
    ReloadPacks,
    /// 打开配置窗口,并直接落在某一页。
