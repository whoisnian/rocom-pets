...
/// 玻璃族 MatCap 高光的叠加量。**要压一档的结构性理由仍然成立**:游戏那边这项还乘着遮罩通道
/// 选出来的高光区,我们没有那张遮罩的语义,只能整片叠。
///
/// 但原来写的理由「满强度叠上去,幽星光那两颗球会泛成一团白」**已经不成立了** —— 那是法线
/// 被写成切线时代的观察(修于 `1daa75e`)。现在实测:GAIN=1.0 时整只亮度中位只从 0.686 抬到
/// 0.715、超过 0.97 的像素占 1.19%,**没有泛白**。所以 0.35 这个具体数字现在是**没有依据的**
/// (改成 1.0 在观感上几乎看不出差别 —— MatCap 只作用在球上,而球在多数取景里很小)。
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    //    **灰度结构本身是对的**。
    //
    // **那对值已经读出来了:亮 = 1.5、暗 = 0.5**(`MI_P_Object_Trans_MatCap` 的 shader
    // 20284,`cb6[60]`/`cb6[61]`,分别是标量 #29 / #32)。它被 `r6 * r11 + r13` 消费
    // (r11 是那对高度渐变色),确认是乘在颜色上的明暗因子。
    //
    // **但代不进来,三条实测**:
    // ① 原值 `mix(0.5, 1.5)`:幽星光圆顶中位从 (238,143,200) 变成 (255,209,255),
    //    对实机 (255,197,242) 的误差和从 111 降到 25 —— 中位数是更准了,但**冲白占比 30.4%**
    //    (实机 0%),圆顶中央糊成一块白;全库回归 `过曝` 从 9 涨到 **109**。
    // ② 只取暗部 `mix(0.5, 1.0)`:全库 `过曝` 反而降到 7,但**对比比实机更强** ——
    //    实机圆顶几乎看不出方向性明暗,而我们的阴影更明显了,更不像。
    // ③ 只取比值 `mix(0.333, 1.0)`:误差 115,比现状 111 **还差** —— 差距根本不在暗部,
    //    在**亮面**:我们封顶 1.0,到不了实机的亮度。
    //
    // 三条合起来指出**真正缺的一环是曝光/显示编码**:实机是先在 HDR 里乘 1.5、再由 shader
    // 尾部的 `movc o0.xyz, (曝光 < 1), sqrt(色 × 曝光), 色` 编码到显示空间,削顶后就成了
    // 实机那种「大片饱和的淡色 + 少量暗部」;我们整条链跑在显示空间、没有这一步,
    // 所以那 1.5 的余量代进来只会硬顶到白。**这一项要等曝光那一环补上再动**,
    // 现在这两个数仍然是猜的(引入于最初的 spike 与 `466326f`,都在法线修复 `1daa75e` 之前)。
