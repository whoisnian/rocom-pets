...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // **固有色链路末尾编码到显示空间**:`sqrt(色 × 曝光)`,照汇编尾部那条
    // `movc o0.xyz, (曝光 < 1), sqrt(色 × 曝光), 色`。
    //
    // **`glow` 不走这条编码,仍留在显示空间。** 它那几层(通用边缘光、玻璃的 MatCap 与
    // 边缘光、星点)的系数都是在显示空间里对着截图标的;`sqrt` 编码会把小值**放大**
    // (0.25 → `sqrt(0.25 × 0.7)` = 0.418,边缘光几乎翻倍),实测全库 `过曝` 从 9 涨到 23。
    // 要把它们也搬进线性,得连着 `GLASS_MATCAP_GAIN` / `GLASS_RIM_GAIN` / `STICK_GAIN`
    // 一起重标 —— 留作下一步,那时才能把 `MatCapColor` 的 HDR 值原样代进来。
    //
    // 输出预乘 alpha(见 render.rs)。**固有色乘 alpha、加上去的光不乘**:高光/星点/边缘光
    // 是打在表面上的光,半透表面照样该有,乘进去会随着变透明一起消失。
    let body_out = sqrt(max(body, vec3<f32>(0.0)) * EXPOSURE);
    return vec4<f32>(body_out * alpha + glow, alpha);
}
