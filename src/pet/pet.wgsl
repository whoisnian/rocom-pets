...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // ② **「实机两端是颜色对」这条是错的,已更正:它是灰度对。**
    //    重解冻结块查实(见 rocom-capture/scripts/uniexpr.py 的 cb 布局那节):那两个槽
    //    (`MI_P_Object_Trans_MatCap` 的 shader 20284 里是 cb6[58]/[59])装的是
    //    `Parameter(下标)`,而下标落在**标量**参数段 —— 也就是一个标量广播成 float4。
    //    所以 `mix(暗, 亮, lit)` 这个**灰度结构本身是对的**,只有那两个数没验过。
    //
    // **这三个数(0.72 / ±0.04 / 下面 rim 的 0.25)都是猜的,而且是在上游法线 bug
    // 修好之前调的**(引入于最初的 spike 与 `466326f`「修掉整只发白」,法线修复是
    // `1daa75e`)。当时看到的 `N·L` 分布是拿**切线**算出来的,和真法线完全是另一套,
    // 所以这几个值的标定基础已经不成立。撤不掉(总得有个值),但**不要把它们当已验证的**。
    //
    // **「按值锚定」这条路对这一项走不通**,试过了:父/根材质的 uniform expression set 里
    // 这对的值是**中性占位**(实测都是 1.0,渲出来等于不打光),真值是每只宠物的 MI 覆盖 ——
    // 要拿到就绕不开参数名。所以它跟着「名字这一关」一起解冻。
