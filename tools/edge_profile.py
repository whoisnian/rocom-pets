...
## 要试「把描边调窄再量一遍」就用 `--outline-scale`

`rocom-pets --render … --outline-scale 0.5` 把描边宽度整体乘 0.5(1 = 包里那份)。
拿它扫一遍就能看出哪一档同时让**暗环面积**与**环的亮度**落到 1 ——
2026-09-05 在 Windows 原生截图上扫的结果记在 findings.md,`×1` 是 1.62 / 0.57,
`×0.4~0.5` 一带两个量同时回到 1。样本只有 4 只,没有照着改标定。
"""
