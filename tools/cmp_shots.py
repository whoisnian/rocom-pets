...
6. **两种场景的截图不能混在一起看**:一拨有地面投影,一拨是平铺卡片、没有任何投影。
   输出按 `lit_scene` 分两拨打印中位数,凡是和光照有关的差异不能混算。注意这条只分场景，
   **不再拿投影猜 shader 排列**；果冻的精确 cooked resource 已证明目标 ES3.1/Low shader
   即使绑定 `MobileDirectionalLight` 也没有 StarStick。
...
9. **截图背景是宠物的「系别色」,不是随便一块底色**:草系绿 (0.32,0.72,0.49)、火系黄橙
   (0.75,0.39,0.20)、水系蓝 (0.30,0.58,0.82)、鬼/暗系紫 (0.55,0.36,0.75)、飞行系青
   (0.25,0.75,0.75)、龙系橙红 (0.75,0.35,0.27)…… 全 32 张逐张量过,按系别成簇。
   两个后果:① **真半透的部位在实机那侧透出来的是这个颜色** —— 春兔耳膜是白色半透,
   看着发绿是背景透过来的,我们这侧要比就得先合成到同一个背景色上(否则会把「白色半透」
   误判成「实机是淡绿、我们画白了」,踩过一次);② **同系宠物天然贴近背景** ——
   波波拉/火神/暮星辰这些「主体与背景亮度差 < 0.08」的,不是巧合而是设计,
   它们的抠图与描边测量永远不可靠,只能换背景重拍。
