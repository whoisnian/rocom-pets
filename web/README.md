...
**计数在前端是两份**(`App.tsx` 的 `stats` 与 `orderStats`),别合回一份:点了下载之后
卡片上的数字要立刻 +1(那是唯一的点击反馈),可**列表不能跟着重排** —— 默认排序就是按下载数,
刚点的那张卡当场跳到第一位、从手指底下溜走,页面也跟着弹回顶部。所以 `stats` 管显示、
跟着乐观 +1 走,`orderStats` 是拉 `/api/stats` 时的那份快照、只用来排序,
于是**新排序下次打开页面才看得到**。
...
- `src/components/PreviewDialog.tsx` —— 弹窗:形态/表情下拉 + 异色开关 + 炫彩三档下拉 +
  动作按钮 + 相机操作。相机绑定照 `OrbitControls` 那套:左键拖转视角,
  右键 / 中键 / Shift+左键拖平移,滚轮缩放(0.5×~5×);触屏单指转、双指同时缩放与平移。
  「复位」把角度、缩放、平移一起还原。
  **弹窗按包 id 重挂**(`key={preview?.id}`)—— 形态、表情、异色、炫彩都是「这一只的」,
  换一只必须从头来:曾经不重挂,于是关掉再开另一只时下拉里还写着上一只的炫彩、
  画出来的却是原样,界面和 wasm 记的不是同一身。
- `src/lib/share.ts` —— 分享链接:`?pet=<包id>&form=<资产名>&face=<表情>&look=<外观>`。
  带参数打开会自动开预览并还原现场,预览开着时地址栏跟着当前选择走,复制地址栏就是链接。
  **默认值一律不写**(链首形态、默认表情、原样外观),于是随手点开一只得到的是干净的
  `?pet=011-鸭吉吉`;认不出来的形态名/表情名**退回默认**而不是报错,链接跨版本也还能用。
  只 `replaceState` 不 push —— 改个配色就压一条历史记录的话,退回列表得按十几次「后退」。
...
`PackCard` 上有 `content-visibility: auto` —— 视口外的一百九十多张卡片跳过样式/布局/绘制,
实测整份列表的全量重排 **2.5~4.2ms**,不跳过是 **11~13.6ms**。换排序、改窗口宽度都要走这一趟,
这个差别留得住。

代价是**占位高度必须和真实高度分毫不差**。视口外的卡片按 `contain-intrinsic-size` 算进文档高,
进了视口换成真实高度 —— 差多少文档就变多少,而**拖着的滚动条滑块就跟着挪**。
症状很好认:刚打开页面往下拖,滑块越拖越落后于鼠标;拖到底之后再上下拖就正常了
(那时每张都量过了);回到顶上一看,滑块比刚打开时更长。曾经占位给 13rem、
而真实高度在 176~248px 之间参差,整页系统性高估 **9%**(15245 → 13978)。

所以卡片钉成定高:形态标签那块固定两行(`h-11`,标签 20px + 行距 4px + 20px),放不下就切。
切掉的不算丢信息 —— 上面「12 种形态」那行本来就写着总数,命中搜索的形态也已经提到最前面。
(也因此**不再缀「+N」**:两行一切,那个数要么被切掉、要么对不上实际显示的条数。)

**`contain-intrinsic-size` 量的是内容盒,边框另算在外面**(`box-sizing: border-box` 在这儿不作数)。
所以填的是「卡片高度减掉上下两条边框」= **198.5px**,不是卡片高度 200px;填 200 的话每张被跳过的
卡片多算 1.33px,六十多行下来就是七十几像素。边框那一项跳过的和画出来的都要加,所以按内容盒填
之后换个屏幕缩放也仍然对得上 —— 实测 1/2/3 列 × dpr 1/1.5/2 六种组合,漂移**全是 0**。

改动卡片里任何一块的高度都要重新量:打开页面记下 `document.documentElement.scrollHeight`,
滚到底再记一次,两个数要一样。
