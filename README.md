...
- 导出器(`exporter/`)：C# + CUE4Parse，从自己的游戏 pak 生成宠物包;
  ...
  **上游 ACL 解码会把轨道接错骨骼**,三条修正:轨道数少于骨骼数却报恒等映射的,
  ...
  有肉的骨骼被恒定挪出绑定姿势就在包里记一行。细节见 design.md「上游 ACL 解码把轨道接错了骨骼」。
  `--probe-anim <资产>` 打印某只的骨架、重定向模式与各段动画的异常平移、`--probe-anim ALL`
  全库普查撞名 —— 待机取到骑乘那一版就是这么查出来的(design.md「待机取到了骑乘那一版动作」);
  `--probe-anim <资产>:<动作>` 把那一段逐根骨骼摊开(轨道号/参考姿势/首帧平移与旋转),
  可丽希亚的右腿就是靠它定位到「上游 ACL 解码整体错位 2 位」的。
