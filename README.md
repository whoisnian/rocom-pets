...
- 外观变异(`src/pet/glassy.rs`)：游戏里那两种稀有外观都做了 —— **异色**(`MDT_SHINING`)
  ...
  **换一张铅绘基色贴图**;龙息帕尔与机幕方舟是 `FlowNoise` + `MixMask` 刷一层 ——
  ...
  炫彩的贴图是全库共用的(13 张 3.5MB),**桌面版构建时烘进二进制** —— 装好就能用,
  不必再摆一份素材目录;导出器正常导包时会把它们写到 `<out>/glassy`,`build.rs` 就读那儿
  ——**导完包重新 `cargo build` 一次就带上了,不必改任何源文件**;找不到时构建会 warn 说明,
  `ROCOM_GLASSY_DIR` 可指定,设成空则不烘。
...
- 下载站(`web/`)：应用本体与宠物包的下载页,整站在 Cloudflare 上 ——
  ...
  可换形态与表情、点按钮做动作、拖着转视角,还能挑**异色**与**炫彩**(和桌面版同一张配置表、
  同一套写法)。点开才加载(wasm 单独一个 chunk,`.rkpet` 按 HTTP Range 只取当前形态,
  炫彩共享贴图挑到哪款取哪几张),要 WebGPU。
