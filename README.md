...
- 导出器(`exporter/`)：C# + CUE4Parse，从自己的游戏 pak 生成宠物包;
  ...
  **NPC 也能导**(`--npc 芙蕾雅,可丽希亚,乐乐` / `--npc-all` / `--npc-list`):
  ...
  (目录名前缀 `npc-`)、一个模型目录一个形态,**运行时不区分 NPC 与宠物**。
  全库 63 个有模型的角色;声音只有 14 个角色有(`NPC_Vo_<英文名>.bnk`,八种情绪,
  内嵌在 bnk 里而不是散落的 wem)。
  **动作用 NPC 自己那套**:运行时靠它活着的十五段按别名表翻过来(`Happy1`→`Happy`、
  坐下三段→`Sleep*`),**其余 `World_*` 有多少导多少** —— 对话五段 + 七种带情绪的、
  抱臂/叉腰/托腮的「起手·循环·收手」、行礼、干杯、挣扎,一个形态 17~56 段
  (宠物那张白名单只有 21 个)。配置窗口那张动作表按形态现算,多出来的那些也能点。
  代价是 glb 从 5MB 涨到 7~16MB,`--npc-clips core` 可退回只导那十五段。
  `Fight_*`(战斗入场演出)**不收** —— 桌宠用不上,而且那一族正是上游 ACL 解码最容易坏的
  (罗兰的 `Fight_Command` 渲出来身体折叠炸开);`CallOut` 走别名表,不受影响。
  整段停在四分之一身高以外的骨骼会被拉回绑定姿势(美术藏道具用的「挪出画面」——
  远行商人的武器在 10.5 米外,留着会把取景撑坏)。
  NPC 那套材质图(`M_C_*`)与宠物的 `M_P_*` 完全不相交,有两处要单独接:
  **两只眼球各占半张贴图而瞳孔贴图只画了一只**(平铺 ×2 那一步在着色器里,导出时烘进 UV0,
  不补就整套内斜视);**自发光/辉光两层的遮罩在 `MatID` 贴图的 B 通道**,不是宠物那边的
  基色 alpha(NPC 的基色 alpha 恒为 1,照宠物那条路走就是整片刷白 —— 恩佐白 ×5、
  露西亚品红 ×10)。两条都从 shader 汇编读出来,见 design.md「NPC 也能出包」那几节。
  ...
  (design.md「待机取到了骑乘那一版动作」);`--probe-anim <资产>:<动作>` 把那一段逐根骨骼摊开
  (轨道号/参考姿势/首帧平移与旋转),可丽希亚的右腿就是靠它定位到「上游 ACL 解码整体错位 2 位」的。
  **轨道数少于骨骼数、映射却是恒等的动作,那份映射一定是错的**(恒等 = 剔掉的是末尾几根,
  而末尾几根往往是整条小腿);这时**换用同资产别的动作带的真映射**,比推断可靠 ——
  菲尔特待机少一只脚就是这么修的,而且它反过来验证了推断法在可丽希亚身上算出的答案一模一样。
  **缩放不跟着错位走**:ACL 三个分量各自剔掉恒定轨道,道具骨的缩放没被剔,下标仍对着骨骼序;
  跟着错位读会把道具的「藏起来」缩放 0.098 扣到小腿上,靴子整只缩进裤腿。
  再加一道只报不改的哨兵:有肉的骨骼被恒定挪出绑定姿势就在包里记一行。
  细节见 design.md「上游 ACL 解码把轨道接错了骨骼」与「NPC 也能出包」那几节。
