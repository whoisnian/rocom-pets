...
- 导出器(`exporter/`)：C# + CUE4Parse，从自己的游戏 pak 生成宠物包;
  ...
  **动作用 NPC 自己那套**:运行时靠它活着的十五段按别名表翻过来(`Happy1`→`Happy`、
  坐下三段→`Sleep*`),**其余资产里有多少导多少** —— 对话五段 + 七种带情绪的、
  抱臂/叉腰/托腮的「起手·循环·收手」、行礼、干杯、战斗入场演出,一个形态 26~67 段
  (宠物那张白名单只有 21 个)。配置窗口那张动作表按形态现算,多出来的那些也能点。
  代价是 glb 从 5MB 涨到 11~20MB,`--npc-clips core` 可退回只导那十五段。
  细节见 design.md「NPC 也能出包」那几节。
